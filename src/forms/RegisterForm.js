  ...
  email: yup
    ...
    .test(
      ...
      async (value) => {
        ...
        const isExist = await apiClient
          .get(`/users/is-exist?email=${value.toLowerCase()}`)
  ...
  const onSubmit = (data) => {
    return apiClient
      ...
      .then((result) => {
        if (result.data) {
          alert(
            "Account has been created, pleaase use your credentials to login"
          );
          history.push("/signin");
        }
      })
      ...
      <Form onSubmit={handleSubmit(onSubmit)}>
        ...
        <Form.Field>
          ...
          {errors.repeatPassword && (
            <Message color="red">{errors.repeatPassword.message}</Message>
          )}
        </Form.Field>
        <Form.Field>
        ...
        <Button type="button" color="yellow" onClick={() => reset({})}>
          Reset
        </Button>
      </Form>
    </Container>
